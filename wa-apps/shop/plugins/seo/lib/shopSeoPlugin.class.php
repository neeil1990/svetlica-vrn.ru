...
class shopSeoPlugin extends shopPlugin
{
	...
	private static $routing_memento;
	...
	public function handleRouteSaveBefore($params)
	{
		$is_new = $params['is_new'];
		$new_route = $params['route'];

		$is_shop_route_updated = ifset($new_route, 'app', null) === 'shop';

		if (!$is_new && $is_shop_route_updated) {
			self::$routing_memento = new shopSeoWaRoutingMemento();

			self::$routing_memento->storeCurrent();
		} else {
			self::$routing_memento = null;
		}
	}

	public function handleRouteSaveAfter($params)
	{
		if (isset(self::$routing_memento)) {
			$handler = new shopSeoWaRouteSaveHandler();

			$handler->handle($params, self::$routing_memento);
		}
