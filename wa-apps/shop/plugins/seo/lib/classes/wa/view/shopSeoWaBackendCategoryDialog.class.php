...
class shopSeoWaBackendCategoryDialog
{
	...
	public function getState($category_id)
	{
		...
		return array(
			...
			'groups_storefronts' => $this->getGroupsStorefronts($category_id),
	...
	private function getGroupsStorefronts($category_id)
	{
		$groups_storefronts = $this->group_storefront_service->getAll();
		$result = array();

		foreach ($groups_storefronts as $group_storefront) {
			$group_storefront_mapped = $this->group_storefront_array_mapper->mapGroupStorefront($group_storefront);
			$group_storefront_mapped['have_personal_value'] = $this->category_settings_service->havePersonal($group_storefront->getId(), $category_id);

			$result[] = $group_storefront_mapped;
		}
