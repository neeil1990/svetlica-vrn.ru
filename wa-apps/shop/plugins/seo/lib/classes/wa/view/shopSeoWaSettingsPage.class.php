...
class shopSeoWaSettingsPage
{
	...
	public function getState($loaded_groups_storefronts_ids, $loaded_groups_categories_ids)
	{
		...
		return array(
			'env' => $this->getEnv(),
			'storefronts' => $this->getStorefrontObjects(),
	...
	private function getStorefrontObjects()
	{
		$storefront_objects = array();

		foreach ($this->storefront_service->getStorefronts() as $storefront) {
			$storefront_objects[] = [
				'label' => waIdna::dec($storefront),
