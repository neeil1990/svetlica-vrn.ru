...
class shopSeoCategorySettingsModel extends waModel implements shopSeoCategorySettingsSource
{
	...
	public function havePersonal($group_storefront_id, $category_id)
	{
		$count = $this
			->select('COUNT(*)')
			->where('group_storefront_id = :sid', array('sid' => $group_storefront_id))
			->where('category_id = :cid', array('cid' => $category_id))
			->fetchField();
